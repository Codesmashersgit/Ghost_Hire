  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'glass-panel !border-x-0 !border-t-0 !rounded-none py-3 shadow-lg' : 'py-5'}`} id="navbar">
      <div className="max-w-[1200px] mx-auto px-6 flex items-center justify-between gap-6">
        ...
        {/* Desktop Nav Links */}
        <div className="hidden md:flex items-center gap-1 bg-black/[0.02] border border-black/[0.04] rounded-full p-1.5 backdrop-blur-md">
          {navLinks.map((link) => (
            <a key={link.label} href={link.href} className="px-4 py-1.5 text-xs font-semibold text-text-secondary rounded-full hover:text-text-primary hover:bg-black/[0.06] transition-all duration-200">
              {link.label}
            </a>
          ))}
        </div>
